...
class Game3D(ShowBase):
 ...
 def cube(self,parent,pos=(0,0,0),scale=(1,1,1),color=(1,1,1,1),hpr=(0,0,0)):
  n=parent.attachNewNode('cube');model=self.loader.loadModel('models/box');model.reparentTo(n);model.setPos(-.5,-.5,-.5);n.setPos(*pos);n.setScale(*(v*2 for v in scale));n.setColor(*color);n.setTextureOff(1);n.setHpr(*hpr);return n
 def world(self):
  self.setBackgroundColor(.31,.64,.76,1);fog=Fog('fog');fog.setColor(.31,.64,.76);fog.setExpDensity(.006);self.render.setFog(fog);a=AmbientLight('ambient');a.setColor((.55,.62,.68,1));self.render.setLight(self.render.attachNewNode(a));sun=DirectionalLight('sun');sun.setColor((1,.91,.72,1));sn=self.render.attachNewNode(sun);sn.setHpr(-35,-55,0);self.render.setLight(sn);cm=CardMaker('grass');cm.setFrame(-220,220,-80,320);grass=self.render.attachNewNode(cm.generate());grass.setP(-90);grass.setColor(.22,.48,.25,1);grass.setTextureOff(1)
  ...
  for _ in range(self.COUNT):
   ...
   for x in (-9.,9.):self.cube(p,(x,0,.16),(.22,self.STEP/self.SCALE,.05),(.85,.20,.13,1))
   for x in (-2.85,2.85):self.cube(p,(x,0,.15),(.07,.45,.025),(.92,.86,.61,1))
   p.flattenStrong();self.road.append(p)
  ...
  if self.camera:self.camera.setPos(0,-16.5,7.3);self.camera.lookAt(0,12,.2)
 def tree(self):
  n=NodePath('tree');n.reparentTo(self.render);self.cube(n,scale=(.2,.2,1),pos=(0,0,.8),color=(.27,.16,.08,1));self.cube(n,scale=(1.1,1.1,2.2),pos=(0,0,2.3),color=(.04,.30,.18,1),hpr=(0,0,45));n.flattenStrong();return n
 def car(self,v,name):
 ...
 def scene(self,dt):
  ...
  for i,p in enumerate(self.road):
   d=i*self.STEP+math.floor((b-i*self.STEP)/road_cycle)*road_cycle
   if d<b-self.STEP*2:d+=road_cycle
   x,y,h=self.pose(d);p.setPos(x,y,0);p.setH(h)
  prop_cycle=len(self.props)//2*110.
  for i,t in enumerate(self.props):
   origin=(i//2)*110.+80.;d=origin+math.floor((b-origin)/prop_cycle)*prop_cycle
   if d<b+35:d+=prop_cycle
   x,y,h=self.pose(d,(-1 if i%2 else 1)*1.45);t.setPos(x,y,0);t.setH(h)
  ...
  if self.camera:self.camera.setPos(self.player.getX()*.35,-16.5,7.3);self.camera.lookAt(self.player.getX()*.25,12,.15)
 def tick(self,task):
  ...
  self.scene(dt);self.hudtext.setText(f'极速公路     {int(self.model.speed):03d} km/h     分数 {self.model.score:06d}     生命 {self.model.life}     最高 {self.best}');self.frames+=1
